...
<main class="accueil-recherche">
            ...
            <h1> Resultats de la recherche de Topics <br> correspondant a: <?php echo $_GET['search']; ?></h1>
            </div>
            <table class="table_topic_recherche">
   <?php
$articles = $bdd->query('SELECT id,message FROM messages ORDER BY id DESC');
if (isset($_GET['search']) and !empty($_GET['search'])) {

    $q = htmlspecialchars($_GET['search']);
    $articles = $bdd->query('SELECT id,message FROM messages WHERE message LIKE "%' . $q . '%" ORDER BY id DESC');
    if ($articles->rowCount() == 0) {
        $articles = $bdd->query('SELECT message FROM messages WHERE CONCAT(message, contenu) LIKE "%' . $q . '%" ORDER BY id DESC');
    }
...
<tr>
   <?php while ($a = $articles->fetch()) {
    ...
    echo "<tr>" . "<td>" . "<a href='sujet.php?id_topic=$id_topic'>" . $a['message'] . "</a>" . "</td>" . "</tr>";
}?>
</tr>
